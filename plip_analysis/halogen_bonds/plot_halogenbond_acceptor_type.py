...
output_pdf = "acceptor_type_barplot.pdf"
counts_output_txt = "acceptor_type_counts.txt"
...
# --------------------------------------
# Load acceptor type data
# --------------------------------------
all_data = []

for lipid_class, folder in lipid_dirs.items():
    txt_path = base_dir / folder / "acceptor_type.txt"
    ...
    if not lines:
        print(f"WARNING: {lipid_class} acceptor_type.txt is empty. Skipping.")
        continue

    df = pd.DataFrame({"acceptor": lines, "class": lipid_class})
    all_data.append(df)

if not all_data:
    raise ValueError("No valid acceptor_type.txt data found.")

merged = pd.concat(all_data, ignore_index=True)

# --------------------------------------
# Count acceptor types
# --------------------------------------
counts = merged.groupby(["class", "acceptor"]).size().unstack(fill_value=0)

# Save raw counts
counts.to_csv(counts_output_txt, sep="\t")
print(f"Saved acceptor-type counts to: {counts_output_txt}")
...
for i, acceptor in enumerate(normalized.columns):
    plt.bar(
        x + i * width,
        normalized[acceptor],
        width=width,
...
plt.title("Normalized Acceptor-Type Frequencies per Lipid Class (max = 1.0)")
