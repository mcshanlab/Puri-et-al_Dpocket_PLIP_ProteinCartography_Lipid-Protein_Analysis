...
# --------------------------------------
base_dir = Path(".")
output_pdf = "pistacking_centdist_violin.pdf"
median_output_txt = "pistacking_centdist_stats.txt"
...
for lipid_class, folder in lipid_dirs.items():
    txt_path = base_dir / folder / "CENTDIST.txt"
    ...
    centdist = pd.to_numeric(pd.Series(lines), errors="coerce").dropna()
    if centdist.empty:
        print(f"WARNING: {lipid_class} has no numeric CENTDIST values. Skipping.")
        continue

    df = pd.DataFrame({"centdist": centdist, "class": lipid_class})
    all_data.append(df)

if not all_data:
    raise ValueError("No valid CENTDIST.txt data found.")
...
data_list = [merged[merged["class"] == c]["centdist"] for c in target_classes]

# --------------------------------------
# EXPORT EXACT VALUES USED IN VIOLIN PLOT
# --------------------------------------
violin_output_file = "centdist_violin_values.tsv"

with open(violin_output_file, "w") as f:
    f.write("class\tcentdist\n")
...
with open(median_output_txt, "w") as f:
    f.write("Lipid class\tMedian (Å)\tSD (Å)\tN\n")
    for cls, med, sd, vals in zip(target_classes, medians, sds, data_list):
        f.write(f"{cls}\t{med:.3f}\t{sd:.3f}\t{len(vals)}\n")
...
plt.ylabel("π-Stacking Center–Center Distance (Å)", fontsize=11)
