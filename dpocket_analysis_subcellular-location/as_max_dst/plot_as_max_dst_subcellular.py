...
for sheet in target_sheets:
    ...
    tmp.columns = ["PDB_ID", "as_max_dst"]

    tmp["PDB_ID"] = tmp["PDB_ID"].apply(clean_dpocket_id)
    tmp["as_max_dst"] = pd.to_numeric(tmp["as_max_dst"], errors="coerce")
    tmp = tmp.dropna(subset=["PDB_ID", "as_max_dst"])
    ...
    # =========================================================
    out_tsv = f"as_max_dst_{sheet}_GO_CC_values.tsv"

    merged[["class", "GO_CC", "as_max_dst"]].to_csv(out_tsv, sep="\t", index=False)
    ...
    # GROUP BY GO
    # =========================================================
    groups = merged.groupby("GO_CC")["as_max_dst"]
    ...
    plt.ylabel("Max. distance of α spheres (Å)")
    plt.title(sheet)
...
# optional: force same axis range for all plots
    ...
    plt.savefig(f"{sheet}_as_max_dst_by_GO_CC.pdf", dpi=300)
    plt.close()

    print(f"saved plot: {sheet}_as_max_dst_by_GO_CC.pdf")
