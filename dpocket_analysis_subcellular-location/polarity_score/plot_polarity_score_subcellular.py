...
for sheet in target_sheets:
    ...
    tmp.columns = ["PDB_ID", "polarity_score"]

    tmp["PDB_ID"] = tmp["PDB_ID"].apply(clean_dpocket_id)
    tmp["polarity_score"] = pd.to_numeric(tmp["polarity_score"], errors="coerce")
    tmp = tmp.dropna(subset=["PDB_ID", "polarity_score"])
    ...
    # =========================================================
    out_tsv = f"polarity_score_{sheet}_GO_CC_values.tsv"

    merged[["class", "GO_CC", "polarity_score"]].to_csv(out_tsv, sep="\t", index=False)
    ...
    # GROUP BY GO
    # =========================================================
    groups = merged.groupby("GO_CC")["polarity_score"]
...
# optional: force same axis range for all plots
    ...
    plt.savefig(f"{sheet}_polarity_score_by_GO_CC.pdf", dpi=300)
    plt.close()

    print(f"saved plot: {sheet}_polarity_score_by_GO_CC.pdf")
