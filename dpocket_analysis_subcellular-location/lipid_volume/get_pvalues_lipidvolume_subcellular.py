...
# --------------------------------------
input_files = glob.glob("lipid_volume_*_GO_CC_values.tsv")

output_file = "lipid_volume_GO_within_lipid_significance.tsv"
...
for file in input_files:
    ...
    df["lig_vol"] = pd.to_numeric(df["lig_vol"], errors="coerce")
    df = df.dropna(subset=["lig_vol", "GO_CC", "class"])
    ...
    for a, b in itertools.combinations(go_groups, 2):
        ...
        group_a = df[df["GO_CC"] == a]["lig_vol"]
        group_b = df[df["GO_CC"] == b]["lig_vol"]
