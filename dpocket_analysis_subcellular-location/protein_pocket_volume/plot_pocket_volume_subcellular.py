...
for sheet in target_sheets:
    ...
    pock_col = [c for c in df.columns if pock_col_keyword in c.lower()]
    if not pock_col:
        continue
    pock_col = pock_col[0]
    ...
    tmp = tmp.dropna(subset=["PDB_ID", "pock_vol"])
    ...
    # =========================================================
    out_tsv = f"pocket_volume_{sheet}_GO_CC_values.tsv"

    merged[["class", "GO_CC", "pock_vol"]].to_csv(out_tsv, sep="\t", index=False)
    ...
    # GROUP BY GO
    # =========================================================
    groups = merged.groupby("GO_CC")["pock_vol"]
...
# optional: force same axis range for all plots
    ...
    plt.savefig(f"{sheet}_pocket_volume_by_GO_CC.pdf", dpi=300)
    plt.close()

    print(f"saved plot: {sheet}_pocket_volume_by_GO_CC.pdf")
