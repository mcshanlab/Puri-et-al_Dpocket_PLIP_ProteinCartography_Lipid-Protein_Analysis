...
prop_col_keyword = "hydrophobicity_score"
...
for sheet in target_sheets:
    ...
    tmp.columns = ["PDB_ID", "hydrophobicity_score"]

    tmp["PDB_ID"] = tmp["PDB_ID"].apply(clean_dpocket_id)
    tmp["hydrophobicity_score"] = pd.to_numeric(tmp["hydrophobicity_score"], errors="coerce")
    tmp = tmp.dropna(subset=["PDB_ID", "hydrophobicity_score"])
    ...
    # =========================================================
    out_tsv = f"hydrophobicity_score_{sheet}_GO_CC_values.tsv"

    merged[["class", "GO_CC", "hydrophobicity_score"]].to_csv(out_tsv, sep="\t", index=False)
    ...
    # GROUP BY GO
    # =========================================================
    groups = merged.groupby("GO_CC")["hydrophobicity_score"]
...
# optional: force same axis range for all plots
    ...
    plt.savefig(f"{sheet}_hydrophobicity_score_by_GO_CC.pdf", dpi=300)
    plt.close()

    print(f"saved plot: {sheet}_hydrophobicity_score_by_GO_CC.pdf")
