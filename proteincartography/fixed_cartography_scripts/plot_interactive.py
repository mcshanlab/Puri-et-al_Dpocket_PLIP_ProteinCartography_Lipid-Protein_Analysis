...
#    # Add Pfam plotting rule (first domain for coloring)
#    plotting_rules["Pfam"] = {
#        "type": "categorical",
#        "fillna": "None",
#        "apply": lambda x: x.split(";")[0] if isinstance(x, str) and x else "None",
#        "textlabel": "Pfam domain",
#    }
...
def generate_scatterplot(
    ...
    category_orders = None  # <- define it at the top
    colors_dict = None
    color_scale = None
    ...
    if plotting_rule["type"] == "categorical":
        ...
        #elif "color_fn" in plotting_rule.keys():
        #    colors_dict = {val: plotting_rule["color_fn"](str(val)) for val in color_keys}
        
        elif "color_fn" in plotting_rule.keys():
        # Unlimited categories (Pfam-safe)
            colors_dict = {val: plotting_rule["color_fn"](str(val)) for val in color_keys}
        ...
        # set category_orders for categorical plots
        category_orders = {col: color_keys}
        color_scale = None

    elif plotting_rule["type"] == "continuous":
        color_scale = plotting_rule.get("color_scale", "viridis")
        colors_dict = None
        category_orders = None

    elif plotting_rule["type"] == "taxonomic":
        taxon_order = plotting_rule.get("taxon_order")
...
def plot_interactive(
    ...
    if output_file is not None:
        ...
        print(f"[plot_interactive] Saved interactive plot → {output_file}")
        export_static_pdf(fig, output_file)  # optional PDF
    if show:
        fig.show()

    return fig

def export_static_pdf(fig, output_file):
    """
    Export a static PDF beside the interactive HTML.
    Example:
    ...
    try:
        ...
        print(f"[plot_interactive] Saved static PDF → {pdf_path}")
    except Exception as e:
        print("[plot_interactive] WARNING: Could not export PDF.")
        print("  → Install kaleido: pip install -U kaleido")
        print("  Error:", e)
