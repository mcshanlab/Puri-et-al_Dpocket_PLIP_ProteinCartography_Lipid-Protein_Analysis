...
# ============================================================
df = pd.read_csv(
    "saccharolipids_plip_interaction_counts.tsv",
    sep="\t"
)
...
# example:
# /path/.../BD6oqx-A-A-N1V1/report.txt -> bd6oqx-a-a-n1v1
# ============================================================
df["PDB_ID"] = (
    df["file"]
    .str.extract(r"(BD[^/]+)/report\.txt")[0]
    .str.lower()
)
...
go_df = go_df.rename(
    columns={
        "protein_Cellular_Component_(GO)": "GO_CC"
    }
)
...
# ============================================================
# OUTLIER ANALYSIS
# collapse to one row per PDB per GO first
# ============================================================
collapsed = (
    merged.groupby(["GO_CC", "PDB_ID"])[order]
    .sum()
    .reset_index()
)
...
for go in collapsed["GO_CC"].unique():
    ...
    for interaction in order:
        ...
        # if no outliers, show top 5 anyway
        if hits.empty:
            out_lines.append(
                "  no outliers; top contributors:"
            )
...
outfile = (
    "saccharolipids_"
    "high_interaction_contributors_"
    "by_subcellular_location.txt"
)
...
for col in order:
    merged[col] = (
        merged[col] / merged["total"]
    )

# ============================================================
# Top 10 GO terms by row count
# ============================================================
top_go = (
...
ax.set_title(
    "Saccharolipids–Protein "
    "Interaction Profiles by "
