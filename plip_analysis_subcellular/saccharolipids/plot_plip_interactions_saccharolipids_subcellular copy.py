...
# -----------------------------
df = pd.read_csv("saccharolipids_plip_interaction_counts.tsv", sep="\t")
...
with open(
    "saccharolipids_high_interaction_contributors_by_subcellular_location.txt",
    "w"
) as f:
    f.write("\n".join(out_lines))
...
ax.set_title(
    "Saccharolipids–Protein Interaction Profiles by Subcellular Location"
)
...
# -----------------------------
plt.savefig(
    "saccharolipids_interaction_profiles_by_subcellular_location.pdf",
    format="pdf",
    bbox_inches="tight"
)
